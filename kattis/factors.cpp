...
void ans_me(LL rm, int id, vi ps)
{
    if(id == 15) {
        int al = 0;
        Rep(i, 15) al += ps[i];
        if(al == 0) return ;
        LL cmb = F[al], num = 1;
        Rep(i, 15) cmb = (cmb * inv[ps[i]]) % MOD;
        Rep(i, 15) while(ps[i]--) num *= prime[i];
        if(ans.find(cmb) == ans.end()) ans[cmb] = num;
        ans[cmb] = min(ans[cmb], num);
        return ;
    }
    ans_me(rm, id + 1, ps);
    int cnt = 0;
...
int main()
{
    F[0] = 1; For(i, 1, 111) F[i] = (F[i - 1]  * i) % MOD;
    Rep(i, 111) inv[i] = InvMod(F[i]);
    sieve();
    vi ps(15, 0);
    ans_me(inF, 0, ps);
