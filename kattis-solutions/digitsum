...
pLL ans_me(int id, bool big)
{
    if(id == len) return mk(0, 1);
    if(dp[id][big].ff != -1) return dp[id][big];
    LL ret = 0;
    LL cnt = 0;
    int lm = big ? in[id] : 9;
    for(int i = 0; i <= lm; i++) {
        ret = ret + i * ans_me(id + 1, big & i == lm).ss + ans_me(id + 1, big & i == lm).ff;
        cnt += ans_me(id + 1, big & i == lm).ss;
    }
    return dp[id][big] = mk(ret, cnt);
}
