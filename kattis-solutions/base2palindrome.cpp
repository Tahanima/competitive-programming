...
bool ckIt(int mid)
{
    LL y = 0;
    int buff[77];
    for(int i = 0; i < mid; i++) buff[i] = buff[len - i - 1] = in[i];
    for(int i = 0; i < len; i++) y = y * 2LL + buff[i];
//    cout << "here : " << y << ' ' << num << '\n';
    return y <= num;

}

LL ans_me(int id, int ad, bool big, bool Lz)
{
    if(id == (len + ad + 1) >> 1) {
        if(big) return ckIt(id);
        return 1;
    }
    LL &ret = dp[id][ad][big][Lz];
    if(ret != -1) return ret;
    ret = 0;
    int lm = big ? in[id] : 1;
    for(int i = 0; i <= lm; i++) {
        if(!i && Lz) ret += ans_me(id + 1, ad + 1, big && i == lm, Lz);
        else ret += ans_me(id + 1, ad, big && i == lm, false);
    }
    return ret;
}
